...
import { FileService } from "src/app/core/http/file.service";
...
export class FileDialogHelper {
  ...
  getNamingDialog( service: FileService, driveId: string, name?: string) {
    return <MyNamingDialog> {
      name: name,
      validators : [Validators.required],
      asynchValidators: [  ValueExistDirective.valid(
         "name", service,  <MyFileGetListQuery> { driveId: driveId }
      )]
    } 
  }
